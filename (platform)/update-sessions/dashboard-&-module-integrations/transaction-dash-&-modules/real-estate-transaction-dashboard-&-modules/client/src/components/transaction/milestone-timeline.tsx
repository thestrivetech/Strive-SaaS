...
export default function MilestoneTimeline({ milestones, onToggleMilestone }: MilestoneTimelineProps) {
  return (
      ...
      <CardContent>
        <div className="space-y-0">
          {milestones.map((milestone, idx) => (
            <div 
              key={milestone.id} 
              className={`flex gap-4 ${onToggleMilestone ? 'cursor-pointer hover-elevate active-elevate-2 p-2 -m-2 rounded' : ''}`}
              onClick={() => onToggleMilestone?.(milestone.id)}
              data-testid={`milestone-item-${idx}`}
            >
