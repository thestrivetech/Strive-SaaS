 ...
 *
 * ⚠️ CURRENTLY DISABLED - Requires Refactoring
 *
 * STATUS: Deferred to future session (documented in Session 16)
 * REASON: Old Express email template system (TemplateEngine) was deleted during migration
 *
 * This script generates static HTML previews for all 8 email template types
 ...
 *
 * MOCK DATA: Preserved below for future implementation
 * EMAIL TYPES: 8 templates (contact form, newsletter, service request, meeting request)
 *
 * TODO (Future Session):
 * - Choose email template solution
 * - Create email templates in /lib/email/templates/
 * - Update this script to use new system
 * - Test HTML generation
 *
 * Related Files:
 * - /lib/email/ (to be created)
 * - /email-previews/ (output directory)
 *
...
// ⚠️ LEGACY IMPORTS - Need to be updated for Next.js
...
// Temporary type definitions (should be moved to proper location)
type ContactFormData = any;
type NewsletterData = any;
type ServiceRequestData = any;
type MeetingRequestData = any;
...
// ⚠️ LEGACY - Needs to be replaced with Next.js email system
